...
class Logger(object):
    ...
    RootLogger = logging.getLogger("cwl_graph_lib")
    LoggerFormat = '[%(levelname)s] [%(asctime)s] [%(name)s] - %(message)s'

    @classmethod
    def setup_root_logger(cls):
        """Sets up the root logger for gdc_graph_lib.  This should only be called
        once
        """
        ...
        formatter = logging.Formatter(Logger.LoggerFormat, datefmt='%H:%M:%S')
